...
export default function AxiosLibrary() {
    ...
    /*
    axios 객체
    - axios 객체는 http method에 해당하는 함수들을 포함하고 있다
    ...
    then(reponse => {
        // response 객체는 status, header, data(body) 값을 포함
        // then 작업은 status code가 400, 500번대가 아닌 response를 받아옴
        console.log(reponse.data);
    })
    .catch(error => {
        // 해당 request 에 대해서 request가 실패하거나, status code가 400, 500번대이면 catch로 AxiosError 객체를 받아옴
        // response가 존재한는 에러는 error 객체에 response가 포함되어 있음
        console.log(error);
    })

    /*
    option 매개변수
    - request의 각종 설정을 조작할 때 사용
    */
    axios.get(GET_URL, {
        headers: {'Authorization': 'Bearer 13452436'}, 
        params: {'time': '1144'},
    })
