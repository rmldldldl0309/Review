...
/*
    ...
    - 원본 배열 변경 X
    - break, continue, ... 부가 적인 작업을 추가 X
    - for 문보다 성능이 떨어짐 but 가독성이 좋고 편리해서 자주 사용
...
/*
    map(콜백 함수) : 콜백 함수의 반환값들로 이루어진 새로운 배열을 생성하여 반환
    - 원본 배열 변경 X
    - forEach > 단순히 요소를 가지고 작업만 진행 (반환 값 X)  /  map > 요소를 가지고 작업한 결과로 배열 새로 만듬(반환 값 O)
*/
