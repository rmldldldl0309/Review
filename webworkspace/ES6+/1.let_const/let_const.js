/*
    let
*/

/*
    블록 레벨 스코프 : 
    - 모든 코드 블럭 내에서  선언된 변수는 해당 코드 블럭 내에서만 사용 가능
*/
{
    var funcLevel = 10;
}
console.log(funcLevel); 

// 블럭 레벨 스코프는 해당 블럭 외부에서 사용 불가능
{
    let blockLevel = 15;
}
// console.log(blockLevel);

/*
    변수 중복 선언 불가 : 
    - 변수를 동일한 이름으로 선언 X
*/
var varVariable = 10;
var varVariable = 15;

let letVariable = 10;
// let letVariable = 15;

/*
    !호이스팅 : var, function 선언문을 최상단에 선언한 것 처럼 동작하는 특성
    - let, 키워드로 선언된 변수는 최상단에서 변수의 선언문까지의 범위를 일시적 사각지대(TDZ: Temporal Dead Zone)에 빠트린다
*/
console.log(varHoisting);
var varHoisting = 10;

//? ReferenceError: Cannot access 'letHoisting' before initialization
//? : let키워드에서도 호이스팅은 일어나지만, 방지책이 존재
// console.log(letHoisting);
// let letHoisting = 10;

// ReferenceError: letHoisting is not defined
// console.log(letHoisting);

/*
    const
*/

/*
    선언과 초기화 : 
    - const는 재할당 불가능
    - 반드시 선언과 동시에 초기화가 이루어져야 한다
*/
const ASSIGNMENT = 10;
//TypeError: Assignment to constant variable.
// ASSIGNMENT = 20;

// 자바스크립트는 컴파일언어가 아니라 빨간줄 원래 안그임 
// SyntaxError: Missing initializer in const declaration
// const INITIALIZATION;

/*
    상수와 객체 : 
    - 코드의 가독성 향상을 위해 많이 사용
    - 객체에 할당하여 참조를 변경하지 못하도록 한다, 속성만 변경가능하도록 함
    - 일반적으로 객체는 상수로 사용
*/
const king = { name : '이성계' };
// TypeError: Assignment to constant variable.
// king = { name : '이방과' };
king.name = '이방과';

/*
    var, let, const
    - ES6 이상을 사용할 수 없을 때 var을 사용
    - 재할당이 발생하는 변수에는 let 사용
    - 재할당이 일어나지 않는 변수에는 const 사용
*/