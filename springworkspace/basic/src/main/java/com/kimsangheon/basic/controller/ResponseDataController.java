...
import com.kimsangheon.basic.dto.ResponseSampleDto;
...
public class ResponseDataController {
    ...
    // @ ResponseEntity : 
    // : Response의 header, status code, status message, data를 조작할 수 있도록하는 클래스
    @GetMapping("/{number}")
    ...
    public ResponseEntity<ResponseSampleDto> getResponseDto() {
        ResponseEntity<ResponseSampleDto> response = ResponseEntity.status(201).body(new ResponseSampleDto("문자열", 10));
        return response;
    }
